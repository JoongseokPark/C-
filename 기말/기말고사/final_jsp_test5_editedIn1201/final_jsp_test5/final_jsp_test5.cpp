...
int main()
{
    ...
    */
    ...
    //5.3번
    distance_tmp = find_max_double(fabs(a - mean), fabs(b - mean), fabs(c - mean));
    if (distance_tmp == fabs(mean - a)) farest = a;
    if (distance_tmp == fabs(mean - b)) farest = b;
    if (distance_tmp == fabs(mean - c)) farest = c;
    distance_tmp = find_min_double(fabs(a - mean), fabs(b - mean), fabs(c - mean));
    ...
    do {
        ...
        if (answer < random2) printf("좀 더 큰 숫자!\n");
        else if (answer > random2) printf("좀 더 작은 숫자!\n");
